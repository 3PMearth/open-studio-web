...
const PhoneInput = ({ className, disabled, onValueChange, value, inputProps, ...rest }: Props) => {
  const locale = useLocale();

  const handleChange = (value: string, country: { name: string; dialCode: string }) => {
    if (onValueChange) {
      onValueChange({
  ...
  return (
    ...
    <div className={`${style.PhoneInput} ${className} ${disabled ? 'pointer-events-none' : ''}`}>
      <ReactPhoneInput
